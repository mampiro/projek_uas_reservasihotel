...
class M_tamu extends CI_Model{
	...
	function cek_pendaftar(){
		return $this->db->query("SELECT * FROM pendaftar where id_pendaftar='".$this->session->userdata('id_pendaftar')."'");
	}

	function cek_temp(){
		return $this->db->query("SELECT a.*, b.* FROM  temp_reservasi a
	...
	function histori_transaksi(){
		...
		from reservasi a
		 ...
		 where a.id_pendaftar='".$this->session->userdata('id_pendaftar')."'  order by a.id_reservasi desc
